...
for k,v in pairs(Beds.interactions) do
	for key, val in pairs(v.position) do
		interactions["beds: "..k] = createInteraction({
			...
			dist = val.dist,
			action = function()
				TriggerServerEvent('rpuk_health:checkIntoHospital', v.bedPostions, v.waitingRoom, dead)
			end
		})
	end
end
