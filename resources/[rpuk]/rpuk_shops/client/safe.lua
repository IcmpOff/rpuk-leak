...
function createSafe(combination, dict, anim)
	RequestStreamedTextureDict( "MPSafeCracking", false )
	RequestAmbientAudioBank( "SAFE_CRACK", false )
	local res
	isMinigame = not isMinigame
	TriggerEvent('mythic_notify:client:SendAlert', { length = 5000, type = 'info', text = "A and D to move. Enter to accept, Backspace to cancel" })
	if isMinigame then
