...
function Progress(action, finish)
	...
	if not IsEntityDead(PlayerPedId()) and not IsEntityPlayingAnim(PlayerPedId(), "misslamar1dead_body", "dead_idle", 3) or mythic_action.useWhileDead then
...
Citizen.CreateThread(function()
	while true do
		...
		if isDoingAction then
			...
			if mythic_action.controlDisables ~= nil then
...
function DisableActions(ped)
	...
	if mythic_action.controlDisables.closeInv then
		TriggerEvent("rpuk_inventory:closeInventory")
		DisableControlAction(0, 170, true) -- Inv Block
		DisableControlAction(0, 171, true) -- Wheel Block
	end
