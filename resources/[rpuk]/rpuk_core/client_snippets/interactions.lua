...
RegisterNetEvent(GetCurrentResourceName() .. ":deleteInteraction")
...
Citizen.CreateThread(function()
	while true do
		...
		for k,v in pairs(interactions) do
			if activeInteractions[k] == nil and getDistance(coords, v.pos) < v.dist and interactions[k] ~= nil and (v.whitelist[1] == nil or isInList(ESX.Player.GetJobName(), v.whitelist)) and (v.blacklist[1] == nil or not isInList(ESX.Player.GetJobName(), v.blacklist)) and not (v.hasToBeInCar and GetVehiclePedIsIn(ped, false) == 0) then
...
function closeToInteraction(k, v)
	Citizen.CreateThread(function()
	  ...
	  local coords = GetEntityCoords(ped)
		...
		while interactions[k] ~= nil and getDistance(coords, v.pos) < v.dist and not interactions[k].refreshInteractions do
...
function getDistance(coords1, coords2)
	-- return #(coords1 - vector3(coords2.x,coords2.y,coords2.z))
...
AddEventHandler(GetCurrentResourceName() .. ":deleteInteaction", function(id)
	interactions[id] = nil
end)