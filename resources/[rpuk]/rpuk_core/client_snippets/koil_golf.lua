--[[
Original Script: NoPixels Koil Public Release
Last Edit: Stealthee, added payment and tightening of the distance checks/optimization
...
Citizen.CreateThread(function()
	while true do
		...
		local courseStart = vector3(-1345.193, 58.679, 54.246)
		if GetDistanceBetweenCoords(coords, courseStart, true) < 50.0 then
			DrawMarker(1, courseStart, 0, 0, 0, 0, 0, 0, 1.5, 1.5, 0.5, 0, 519, 0, 105, 0, 0, 2, 0, 0, 0, 0)			
			if GetDistanceBetweenCoords(coords, courseStart, true) < 1.5 then
				if golfplaying then
					ESX.ShowHelpNotification("Press ~g~~INPUT_CONTEXT~~s~ to end golf.")
				else
					ESX.ShowHelpNotification("Press ~g~~INPUT_CONTEXT~~s~ to start golf (£1500).")
				end
				if (IsControlJustReleased(1, 38)) then
					...
					if (golfplaying) then
						...
						ESX.TriggerServerCallback('rpuk_golf:startPlay', function(result)
							if result then
								startGolf()
								ESX.ShowAdvancedNotification("Los Santos Golf Club", "Instructions", "Head to the start(Flashing on your map) End holes are marked on your map\nPress/Hold E to swing\nA & D to roatate\nY to swap club", 'CHAR_PROPERTY_GOLF_CLUB', 1)
							else
								ESX.ShowAdvancedNotification("Los Santos Golf Club", "Payment Declined", "I suggest you find a job and get some money in your bank before trying to learn golf.", 'CHAR_CASTRO', 1)
							end
						end)
					end
				end				
			end
		elseif GetDistanceBetweenCoords(coords, courseStart, true) > 500.0 and golfplaying then
			endgame()
		end		
	end
end)
...
function endShot()
	...
	if distance < 1.5 then
		ESX.ShowAdvancedNotification("Los Santos Golf Club", "Impressive. Ball in range!", "", 'CHAR_PROPERTY_GOLF_CLUB', 1)
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end
	if golfstrokes > 12 then
		ESX.ShowAdvancedNotification("Los Santos Golf Club", "Are you new?", "12+ Shots... You will have to do better than that to get into the high society.", 'CHAR_PROPERTY_GOLF_CLUB', 1)	
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end
...
function dropShot()
	...
	while doingdrop do
		...
		Citizen.Wait(0)
		local distance = GetDistanceBetweenCoords(GetEntityCoords(mygolfball), GetEntityCoords(PlayerPedId()), true)
		local distanceHole = GetDistanceBetweenCoords(holes[golfhole]["x2"],holes[golfhole]["y2"],holes[golfhole]["z2"], GetEntityCoords(PlayerPedId()), true)
