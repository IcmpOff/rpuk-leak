...
Citizen.CreateThread(function()
	while true do
		...
		DisableControlAction(0, 36, true) -- disable CTRL left
		...
		if Config.EnableXtoCancel then if IsControlPressed(0, 20) then EmoteCancel() end end
	end
end)
...
function EmoteCommandStart(source, args, raw)
	if #args > 0 then
		if ESX.PlayerData.jail_state then
			TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = "Sorry I cant let you do that" })
			return
		else
