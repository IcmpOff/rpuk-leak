ESX.RegisterCommand('race', 'user', function(xPlayer, args, showError)
	-- TODO fix&rewrite whatever mutex is, and why is it encoded on the player class file?
	local mutexWhatever = json.decode(xPlayer.getmutexjobdata())
	if xPlayer.isAceAllowed('race.command') or (mutexWhatever and mutexWhatever.richman > 1) then
		xPlayer.triggerEvent('streetraces:raceCommandEntered', args)
	else
		showError('You are not authorized to use this command')
	end
end, false, {help = 'Race command (TODO improve docs)'})
		...
		for index, race in pairs(races) do
			...
			elseif (race.finishTime ~= 0) and (time > race.finishTime + race.finishTimeout) then
				...
				for _, player in pairs(players) do
					notifyPlayer("mythic", player, "DNF (timeout)")
				end
	...
	for index, race in pairs(races) do
		...
		if source == race.owner and time < race.startTime then
			...
			for _, player in pairs(race.players) do
				...
				local msg = "Race cancelled, all participants refunded"
				notifyPlayer("chat", player, "RACE ANNOUNCEMENT", msg)
			end
...
AddEventHandler("StreetRaces:joinRace_sv", function(index)
	...
	if playerMoney >= amount then
		...
		notifyPlayer("mythic", source, "RACE: ", msg)
	end
end)
	...
	for index, player in pairs(players) do
		if source == player then
			...
			if race.finishTime == 0 then
				...
				local msg = ("%s won the race with time [%02d:%06.3f]"):format(getName(source), timeMinutes, timeSeconds)
				notifyPlayer("chat", -1, "RACE ANNOUNCEMENT", msg)
			else
				-- Loser, send notification to only the player
				local msg = ("You lost with time [%02d:%06.3f]"):format(timeMinutes, timeSeconds)
				notifyPlayer("mythic", source, "RACE ANNOUNCEMENT", msg)
			end
...
AddEventHandler("StreetRaces:saveRace_sv", function(name, checkpoints)
	...
	notifyPlayer("mythic", source, "RACE ADMIN: ", msg)
end)
