Citizen.CreateThread(function()
	...
	if ESX.Player.GetInventory().radio and ESX.Player.GetInventory().radio.count > 0 then
		TriggerEvent('Radio.Set', true)
	end
end)

RegisterNetEvent("esx:addInventoryItem")
AddEventHandler("esx:addInventoryItem", function(item, count) --Listen for adding the radio
	if item == "radio" and count >= 1 then
		TriggerEvent('Radio.Set', true)
	end
end)
...
Radio.Commands = {
	{
		...
		Handler = function(src, args, raw)
			...
			local isDead = IsEntityPlayingAnim(PlayerPedId(), 'misslamar1dead_body', 'dead_idle', 3)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if Radio.Open then
			...
			if Radio.On then
				if Cfg.radioPressed and not isPlayingBroadcastAnim then
				...
				elseif not Cfg.radioPressed and isPlayingBroadcastAnim then
					StopAnimTask(playerPed, broadcastDictionary, broadcastAnimation, -4.0)
				end
			end

			-- Play default animation if not broadcasting
			if not Cfg.radioPressed and not isPlayingOpenAnim then
			...
			if IsControlJustPressed(0, radioConfig.Controls.Toggle.Key) then
				...
				if Radio.On then
					SendNUIMessage({sound = "radio_on", volume = 0.3})
					Radio:Add(radioConfig.Frequency.Current)
				else
					SendNUIMessage({sound = "radio_off", volume = 0.5})
					Radio:Remove()
				end
			end
			...
			if radioConfig.AllowRadioWhenClosed then
				if Radio.Has and Radio.On and Cfg.radioPressed and not isPlayingBroadcastAnim then
					RequestAnimDict(broadcastDictionary)
...
RegisterKeyMapping('radio_frequp', 'Service Radio Freq +', 'keyboard', 'pageup')
RegisterCommand('radio_frequp', function(source)
	if ESX.Player.GetJobName() == "police" or ESX.Player.GetJobName() == "ambulance" or ESX.Player.GetJobName() == "gruppe6" then
		if Radio.Has and Radio.On then
			Radio:Increase()
			SendNUIMessage({ sound = "radio_off", volume = 0.5})
		end
	end
...
RegisterKeyMapping('radio_freqdown', 'Service Radio Freq -', 'keyboard', 'pagedown')
RegisterCommand('radio_freqdown', function(source)
	if ESX.Player.GetJobName() == "police" or ESX.Player.GetJobName() == "ambulance" or ESX.Player.GetJobName() == "gruppe6" then
		Radio:Decrease()
		SendNUIMessage({ sound = "radio_off", volume = 0.5})
	end
...
RegisterKeyMapping('radio_panic', 'Service Radio Panic (Shift + )', 'keyboard', 'b')
RegisterCommand('radio_panic', function(source)
	if Radio.Has and Radio.On then
		if IsControlPressed(0, 21) then
			if (ESX.Player.GetJobName() == "police" or ESX.Player.GetJobName() == "ambulance" or ESX.Player.GetJobName() == "gruppe6") then			
				...
				TaskPlayAnim(PlayerPedId(), "random@arrests", "generic_radio_enter", 8.0, 2.0, 1000, 50, 2.0, 0, 0, 0 )
				local playerPed = PlayerPedId()
				...
				elseif ESX.Player.GetJobName() == 'ambulance' then
					faction = "NHS"
				elseif ESX.Player.GetJobName() == 'gruppe6' then
					faction = "Gruppe 6"
				end
				local message = faction .. ' - ' .. ESX.Player.GetName() .. ' has pressed their panic alarm!'
				ESX.ShowNotification('Panic Alarm Activated!')
				TriggerServerEvent('rpuk_alerts:sNotification', {notiftype = "panic", officerName = true, frequency = radioConfig.Frequency.CurrentIndex})
				SendNUIMessage({ sound = "radio_panic", volume = 0.1}) --Play sound for the client
				TriggerServerEvent('rp-radio:panicAlarm') --Play sound for the police
			end
		end
	end
end)

RegisterNetEvent("rp-radio:panicSound")
AddEventHandler("rp-radio:panicSound", function()
	SendNUIMessage({ sound = "radio_panic", volume = 0.1}) --Play sound for the client
end)